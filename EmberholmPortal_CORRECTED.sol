 ...
 *
 * Version: 3.1 (Corrected - Compatible with Backend)
 * Network: Base Mainnet
 * Audit: 2025-11-24
 *
 * CHANGES FROM v3.0:
 * - Fixed equipment slots: boots → trinket (matches metadata)
 * - Changed ItemStats to match D&D game system (str/dex/con/int/wis/cha)
 * - Added unstake cooldown for missionManager security
...
interface IEmberholmItems {
    ...
    // 🔥 UPDATED: Changed to match game's D&D stat system
    struct ItemStats {
        uint8 strBonus;          // Strength bonus (was attackBonus)
        uint8 dexBonus;          // Dexterity bonus (was speedBonus)
        uint8 conBonus;          // Constitution bonus (was defenseBonus)
        uint8 intBonus;          // Intelligence bonus (NEW)
        uint8 wisBonus;          // Wisdom bonus (NEW)
        uint8 chaBonus;          // Charisma bonus (NEW)
        uint8 powerBonus;        // Power bonus (derived stat)
        uint8 successRateBonus;  // Mission success rate bonus (was auraBonus)
...
contract EmberholmPortal is ERC721, ERC2981, Ownable, Pausable {
    ...
    // 🔥 NEW: Unstake cooldown for security
    ...
    struct TokenInfo {
        ...
        address owner;
        bool isStaked;
        uint256 stakedSince;  // 🔥 NEW: Track stake duration
        // Note: guild, name, rank, and achievements are in metadata (backend manages)
    }
    ...
    mapping(uint256 => uint256) public lastUnstakeTime;  // 🔥 NEW: For cooldown
    ...
    // 🔥 FIXED: Changed "boots" to "trinket" to match metadata
    ...
    event MintPriceUpdated(uint256 newPrice);  // 🔥 NEW
    event BaseURIUpdated(string newBaseURI);   // 🔥 NEW
    ...
    function getTokenInfo(uint256 tokenId) public view returns (TokenInfo memory) {
        ...
        return TokenInfo({
            ...
            stakedSince: stakeTimestamp[tokenId]  // 🔥 NEW
        });
    }
     ...
     * 🔥 USAGE: Backend should call this when mission starts
     */
    function stakeToken(uint256 tokenId) external whenNotPaused {
     ...
     * 🔥 UPDATED: Added cooldown for missionManager to prevent abuse
     * 🔥 USAGE: Backend should call this when mission completes
     */
    function unstakeToken(uint256 tokenId) external whenNotPaused {
        address owner = ownerOf(tokenId);
        require(
        ...
        // 🔥 NEW: Enforce cooldown for missionManager (security)
    ...
    // Note: Achievements, ranks, and guild leadership moved to backend (FREE, appears in OpenSea metadata)
    ...
    function getTotalStats(uint256 tokenId) external view returns (
        ...
        for (uint i = 0; i < equipmentSlots.length; i++) {
            ...
            if (itemId != 0) {
                ...
                totalStr += stats.strBonus;
                totalDex += stats.dexBonus;
                totalCon += stats.conBonus;
                totalInt += stats.intBonus;
                totalWis += stats.wisBonus;
                totalCha += stats.chaBonus;
                totalPower += stats.powerBonus;
                totalSuccessRate += stats.successRateBonus;
            }
        }
    }

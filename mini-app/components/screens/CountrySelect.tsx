...
// Complete list of all 195 countries with flags (emoji)
const COUNTRIES = [
  ...
  { code: 'OTHER', name: 'Other / Prefer not to say', flag: '🌍' },
].sort((a, b) => {
...
export function CountrySelect({ onSelect, onBack }: CountrySelectProps) {
  ...
  return (
    <div className="screen-view flex flex-col min-h-screen p-4">
        ...
        <div className="country-grid">
          {filteredCountries.map((country, index) => (
            <motion.button
              ...
              transition={{ delay: Math.min(0.01 * index, 0.5) }}
      ...
      {/* Back button */}
      {onBack && (
        <button onClick={onBack} className="back-btn mt-2">
          ← BACK
        </button>
      )}
    </div>
